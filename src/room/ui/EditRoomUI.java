...
public class EditRoomUI extends JFrame
{
	...
	public EditRoomUI(Room roomSel) throws SQLException, UserNotFoundException 
	{
		...
		roomPan.add(lblName, gbc_lblName);
			...
			//textfield
		final JTextField txtFName = new JTextField(room.getNameRoom());
		...
		roomPan.add(lblSurface, gbc_lblSurface);
			...
			//JSpinner
		SpinnerModel sufaceSpinerModel = new SpinnerNumberModel(room.getSurfaceRoom(),SURFACEMIN,SURFACEMAX,1);
		...
		{
			cBoxTypeRoom.setSelectedIndex(0);
		}
		if (room.getTypeRoom()==1)
		{
			cBoxTypeRoom.setSelectedIndex(1);
		}
		...
		final ArrayList<Accessory> accessoriesSel = room.getListAccessory();
		...
		for (int i=0;i<accessoriesSel.size();i++)
		{
			accessories.remove(accessoriesSel.get(i));
		}
